...
MaerPlotBR = ggplot(MaerBR %>%
  ...
  #geom_signif(comparisons = list(c("Control", "Phe-Az"), c("Control", "Prop-Lys"), c("Control", "Prop-Tyr")), step_increase =  .1, 
...
MaerBR = filter(Maer, Treatment == "A405+BODIPY")
MaerBR$Feeding[MaerBR$Feeding == "No Feed"] = "Control"

PCC6803BR = filter(PCC6803, Treatment == "A405+BODIPY")
PCC6803BR$Feeding[PCC6803BR$Feeding == "No Feed"] = "Control"

P371BR = filter(P371, Treatment == "A405+BODIPY")
P371BR$Feeding[P371BR$Feeding == "No Feed"] = "Control"

WTBR = filter(WT, Treatment == "A405+BODIPY")
WTBR$Feeding[WTBR$Feeding == "No Feed"] = "Control"

ApnkoBR = filter(Apnko, Treatment == "A405+BODIPY")
ApnkoBR$Feeding[ApnkoBR$Feeding == "No Feed"] = "Control"

CptdelBR = filter(Cptdel, Treatment == "A405+BODIPY")
CptdelBR$Feeding[CptdelBR$Feeding == "No Feed"] = "Control"

MVBR = filter(MV, Treatment == "A405+BODIPY")
MVBR$Feeding[MVBR$Feeding == "No Feed"] = "Control"

dD2BR = filter(dD2, Treatment == "A405+BODIPY")
dD2BR$Feeding[dD2BR$Feeding == "No Feed"] = "Control"
...
MaerPlotBR = ggplot(MaerBR %>%
                      group_by(Feeding) %>%
                      mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 1.80) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank()) 

MaerPlotBR = MaerPlotBR + #geom_signif(comparisons = list(c("No Feed", "Phe-Az"), c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #map_signif_level = TRUE) + 
  ggtitle("M. aeruginosa")


MaerPlotBR = MaerPlotBR + theme(plot.title = element_text(face = "italic", hjust = 0.5)) 
...
PCC6803PlotBR = ggplot(PCC6803BR %>%
                         ...
                         mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 5.07) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank()) 

PCC6803PlotBR = PCC6803PlotBR + #geom_signif(comparisons = list(c("No Feed", "Phe-Az"), c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #   map_signif_level = TRUE) + 
  ggtitle(expression(paste(italic("Synechocystis sp."), "PCC6803")))


PCC6803PlotBR = PCC6803PlotBR + theme(plot.title = element_text(hjust = 0.5))
...
P371PlotBR = ggplot(P371BR %>%
                      ...
                      mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 3.31) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank())  

P371PlotBR = P371PlotBR + #geom_signif(comparisons = list(c("No Feed", "Phe-Az"), c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #map_signif_level = TRUE) +
  ggtitle(expression(paste(italic("P. agardhii")," No371/1")))

P371PlotBR = P371PlotBR + theme(plot.title = element_text(hjust = 0.5))
...
WTPlotBR = ggplot(WTBR %>%
                    ...
                    mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +  
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 1.94) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank())  

WTPlotBR = WTPlotBR + #geom_signif(comparisons = list(c("No Feed", "Phe-Az"), c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #     map_signif_level = TRUE) + 
  ggtitle(expression(paste(italic("P. agardhii")," CYA126/8 WT")))

WTPlotBR = WTPlotBR + theme(plot.title = element_text(hjust = 0.5))
...
ApnkoPlotBR = ggplot(ApnkoBR %>%
                       ...
                       mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 3.59) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank()) 

ApnkoPlotBR = ApnkoPlotBR + #geom_signif(comparisons = list(c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #map_signif_level = TRUE) + 
  ggtitle(expression(paste(italic("P. agardhii")," CYA126/8"~Delta~"apnC")))

ApnkoPlotBR = ApnkoPlotBR + theme(plot.title = element_text(hjust = 0.5))
...
CptdelPlotBR = ggplot(CptdelBR %>%
                        ...
                        mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 2.03) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank()) 

CptdelPlotBR = CptdelPlotBR + #geom_signif(comparisons = list(c("No Feed", "Phe-Az"), c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #   map_signif_level = TRUE) + 
  ggtitle(expression(paste(italic("P. agardhii")," CYA126/8"~Delta~"ociA")))

CptdelPlotBR = CptdelPlotBR + theme(plot.title =element_text(hjust = 0.5))
...
MVPlotBR = ggplot(MVBR %>%
                    ...
                    mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 2.93) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank()) 

MVPlotBR = MVPlotBR + #geom_signif(comparisons = list(c("No Feed", "Phe-Az"), c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #map_signif_level = TRUE) +
  ggtitle(expression(paste(italic("P. agardhii")," CYA126/8"~Delta~"mvdC")))

MVPlotBR = MVPlotBR + theme(plot.title = element_text(hjust = 0.5))
...
dD2PlotBR = ggplot(dD2BR %>%
                     ...
                     mutate(BR = mean(Ratio.Ch0.2)), aes(x=Feeding, y = Ratio.Ch0.2)) + ylim(0,9) +  
  geom_boxplot(aes(fill = BR), outlier.shape = NA) +
  scale_fill_gradient2(low= "grey", mid = "grey", high= "#66CCFF", midpoint = 2.26) + theme_test() +
  theme(axis.title.x=element_blank(), axis.ticks.x=element_blank(), axis.title.y=element_blank(), axis.ticks.y = element_blank()) 

dD2PlotBR = dD2PlotBR + #geom_signif(comparisons = list(c("No Feed", "Phe-Az"), c("No Feed", "Prop-Lys"), c("No Feed", "Prop-Tyr")), step_increase =  .1, 
  #map_signif_level = TRUE)+ 
  ggtitle(expression(paste(italic("P. agardhii")," CYA126/8"~Delta~"mcyD")))


dD2PlotBR = dD2PlotBR + theme(plot.title = element_text(hjust = 0.5))
